...
public class Tree extends Entity{
	...
	public void checkPoints()
	{
		...
		{
			AllEntities.setMap(x+j,y-1,id);
			AllEntities.setMap(x+j,y,id);
			AllEntities.setMap(x+j,y+1,id);
		}
		if(isEclosed)
		{
			for (int i = 0;i<Stroncs.length; i++)
			{
				for(int j = 0; j<sizetroncx; j++)
				{
					...
					{
						AllEntities.setMap(x+j,y+i*sizetroncy+k,id);
					}
				}
				...
				if(troncType[i] == 0)
				{
					for(int j = 0; j<sizebranchex; j++)
					{
						...
						{
							AllEntities.setMap(x-j-1,y+i*sizetroncy+k,id);
						}
					}
				}
				else if(troncType[i] == 1)
				{
					for(int j = 0; j<sizebranchex; j++)
					{
						for(int k = 0; k<sizebranchey; k++)
						{
							AllEntities.setMap(x+j+sizetroncx,y+i*sizetroncy+k,id);
						}
					}
				}
				if(i == Stroncs.length-1)
				{
					for(int j = 0; j<sizecimex; j++)
					{
						...
						{
							AllEntities.setMap(x+j-(sizecimex-sizetroncx)/2,y+(i+1)*sizetroncy+k,id);
						}
					}
				}
			}
	...
	public void checkEclosion()
	{
		...
		{
			boolean Ok = true;
			int curid = AllEntities.getItem(x, y);
			for(int i = 0; i<9; i++)
			{
				for(int j = 3; j<25; j++)
				{
					if(!Map.instance.mainLayer.getBloc((i+x-4),j+y).air || (AllEntities.getItem(x+i-4, y+j)!=-1 && AllEntities.getItem(x+i-4, y+j)!=curid))Ok = false;
				}
			}
			for(int i = 0; i<3; i++)
			{
				for(int j = 0; j<3; j++)
				{
					if(!Map.instance.mainLayer.getBloc((i+x-1),j+y).air || (AllEntities.getItem(x+i-1, y+j)!=-1 && AllEntities.getItem(x+i-1, y+j)!=curid))Ok = false;
				}
			}
			/*for (int i = 0; i < GameScreen.entities.trees.treeList.size(); i++)
			{
				if(GameScreen.entities.trees.treeList.get(i).isEclosed)
				{
					if(Math.abs(GameScreen.entities.trees.treeList.get(i).x-x)<6 && Math.abs(GameScreen.entities.trees.treeList.get(i).y-y)<19) Ok = false;
				}
			}*/
			if(Ok)
			{
