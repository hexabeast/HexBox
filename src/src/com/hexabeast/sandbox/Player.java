...
public class Player extends Entity{
	...
	public void Update()
	{
		if(!Main.pause)
		{
			...
			//if(Gdx.input.isButtonPressed(Input.Buttons.MIDDLE))Map.instance.lights.tempLight(x+w/2, y+h/2,0.65f,0.65f,0.65f);
			if(AllEntities.getType(Tools.floor(middle.x/16), Tools.floor(middle.y/16)-1) == AllEntities.furnituretype)
			{
				int id = AllEntities.getItem(Tools.floor(middle.x/16), Tools.floor(middle.y/16)-1)-Constants.treelimit;
				if(GameScreen.entities.furnitures.furnitureListAll.get(id).container)
				{
					if(id != GameScreen.inventory.chestid || !GameScreen.inventory.chestopen)
					{
						GameScreen.inventory.putinchest(id);
					}
					GameScreen.inventory.chestid = id;
	...
	if(sword && press)
	{
		if(Gdx.input.isButtonPressed(Input.Buttons.LEFT))
		{
			...
			while(!finished)
			{
				...
				{
					int id = AllEntities.getItem(Tools.floor((launcherCoordBis.x+awx)/16), Tools.floor((launcherCoordBis.y+awy)/16))-Constants.chestlimit;
					...
					{
						if(rects.get(i).contains((launcherCoordBis.x+awx)-GameScreen.entities.mobs.mobListAll.get(id).x, (launcherCoordBis.y+awy)-GameScreen.entities.mobs.mobListAll.get(id).y))
						{
							touched = true;
							break;
						}
					...
					if(touched)
					{
						...
						GameScreen.entities.mobs.mobListAll.get(id).damage(currentDamage,0.5f,launcherCoordBis.x+awx, launcherCoordBis.y+awy);
						currentDamage *= 0.6f;
					}
				}
