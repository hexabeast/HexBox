...
{	
	...
	public static OrthographicCamera camera = new OrthographicCamera(1280,720);
	...
	private void Clear()
	{
		Gdx.gl.glClearColor(85/255f,156/255f,211/255f,1f);
	...
	private void TeleportPlayerOtherSide()
	{
		if(noLimit)
		{
			if(camiddle.x<0)
			{
				...
				//Map.instance.lights.traverseLeft();
			}
			if(camiddle.x>=(Map.instance.width)*16)
			{
				...
				camera.position.set(camera.position.x-(Map.instance.width)*16, camera.position.y, camera.position.z);
				player.currentGrapple.tpOtherSide();
				//Map.instance.lights.traverseRight();
			}
	...
	private void UpdateAndDrawRenderer(){
		...
		//batch.draw(Shaders.instance.fbo.getColorBufferTexture(), camera.position.x, camera.position.y);
		
		
		
		Shaders.instance.setDefaultShader();
		//if(fullBright)batch.draw(Shaders.instance.lightmap, camera.position.x-200, camera.position.y-200,512,512);
		batch.end();
		...
		if(inventory.hidden && !Main.pause)
		{
			...
			if(tree || furniture || block)
			{
				...
				else if(block)
				{
					...
					else if(AllEntities.getItem(px, py) != -1)ok = false;
	...
	public void MouseInputs()
	{
		...
		{
			//PUTCELL
			if(!abort && inventory.hidden)ModifyTerrain.instance.PutCell(Gdx.input.getX(), Gdx.input.getY(), player.currentCellID,player.currentCellState, Map.instance.mainLayer,time,true);
		}
		else if(Gdx.input.isButtonPressed(Input.Buttons.RIGHT))
		{
			...
			if(!abort && inventory.hidden && !backPlaced)ModifyTerrain.instance.PutCell(Gdx.input.getX(), Gdx.input.getY(),  player.currentCellID,player.currentCellState, Map.instance.backLayer,time,false);
		}
	}
