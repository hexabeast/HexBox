...
public class MapLayer {
	...
	{
		if(getBloc(i,j).torch && torchState>1)
		{	
			setBloc(i, j, getBloc(i,j), (byte) ((int)getState(i,j)+1), false);
			if(getState(i,j)>5)setBloc(i, j, getBloc(i,j), 0, false);
		}
		if(getBloc(i,j).animated && animState>1)
		{
			setBloc(i, j, getBloc(i,j).next, getState(i,j), false);
		}
	}
	...
	public void renderBlock(SpriteBatch batch, Color col, int i, int j)
	{
			...
			{
				batch.draw(TextureManager.instance.TextureRegions[getBloc(i,j).Id][getState(i,j)%10], i*16, j*16,8,8, 16, 16,1,1,angle);
			}
			else
			{
					...
					if(tooDif)
					{
						for(int l = 0; l<2; l++)
							for(int m = 0; m<2; m++)
							{
								...
								int srcX = TextureManager.instance.CoordinatesBlocs[getBloc(i,j).Id].x+(int)((getState(i,j)%10)%3)*10+4*l;
								int srcY = TextureManager.instance.CoordinatesBlocs[getBloc(i,j).Id].y+(int)((int)((getState(i,j)%10)/3))*10+4*m;
								int srcWidth = 4;
								int srcHeight = 4;
	...
	public void renderOldBlock(SpriteBatch batch, Color col, int i, int j)
	{
			...
			if (Parameters.i.fullBright)
			{
				batch.draw(TextureManager.instance.TextureRegions[getBloc(i,j).Id][getState(i,j)], i*16, j*16, 16, 16);
			}
			else
			{
				color.set(Tools.getShadowColor(i,j));
				...
				else
				{
					...
					if(tooDif)
					{
						for(int l = 0; l<2; l++)
							for(int m = 0; m<2; m++)
							{
								...
								int srcX = TextureManager.instance.CoordinatesBlocs[getBloc(i,j).Id].x+(int)(getState(i,j)%9)*10+4*l;
								int srcY = TextureManager.instance.CoordinatesBlocs[getBloc(i,j).Id].y+(int)((int)(getState(i,j)/9))*10+4*m;
								int srcWidth = 4;
								int srcHeight =  4;
					...
					{
						batch.setColor(color.x*col.r,color.y*col.g,color.z*col.b,1);
						batch.draw(TextureManager.instance.TextureRegions[getBloc(i,j).Id][getState(i,j)], i*16, j*16, 8, 8, 16, 16, 1, 1, angle);
					}
				}
			}
	}
	...
	public void draw(SpriteBatch batch)
	{
		...
		for(int i = minX; i<=maxX; i++)
		{
			for(int j = minY; j<=maxY; j++)
			{
				if(getBloc(i,j).Id != 0)
				{
					if(isMain || !Tools.isFull(Map.instance.mainLayer.getBloc(i,j).Id,Map.instance.mainLayer.getState(i,j)) || Map.instance.mainLayer.getBloc(i,j).transparent)
					{
						if(!isMain || !getBloc(i,j).needBack)
						{
							if(getBloc(i,j).oldTexture)renderOldBlock(batch,col, i, j);
				...
				if(!isMain)
				{
					if(Map.instance.mainLayer.getBloc(i,j).needBack)
					{
						...
						if(getBloc(i,j).oldTexture)Map.instance.mainLayer.renderOldBlock(batch,Color.WHITE, i, j);
			...
			for(int i = minX; i<=maxX; i++)
			{
				for(int j = minY; j<=maxY; j++)
				{
					if(!isMain)
					{
						...
						if(!Tools.isFull(Map.instance.mainLayer.getBloc(i,j).Id,Map.instance.mainLayer.getState(i,j)) || Map.instance.mainLayer.getBloc(i,j).transparent)drawTransition(batch,col, i, j);
					}
					else
					{
