...
public class MapGenerator {
	...
	public void GenerateMap()
	{
		...
		//IRON
		float[][] perlinIron = noise.generateGradient(width, height, 8, 8);
		for(int i = 0; i<width; i++)
		{
		...
		//IRON2
		float[][] perlinIron2 = noise.generateGradient(width, height, 16, 16);
		for(int i = 0; i<width; i++)
		{
			for(int j = limit-30; j<height; j++)
			{
		...
		float[][] perlinRare = noise.generateGradient(width, height, 33, 33);
		for(int i = 0; i<width; i++)
		{
			for(int j = 0; j<height; j++)
			{
				if(perlinIron[i][j]+perlinIron2[i][j]>0.78)
				{
					Map.instance.mainLayer.blocs[i][j] = AllBlocTypes.instance.Iron;
				}
				if(perlinIron[i][j]+perlinIron2[i][j]<-0.85)
				{
					Map.instance.mainLayer.blocs[i][j] = AllBlocTypes.instance.Gold;
				}
				if(perlinIron[i][j]*2*perlinIron2[i][j]+perlinRare[i][j]<-0.72)
				{
		...
		//DIRT
		float[][] perlinDirt = noise.generateGradient(width, height, 25, 25);
		for(int i = 0; i<width; i++)
		{
		...
		//FOSSILE
		float[][] perlinFossile = noise.generateGradient(width, height, 15, 15);
		for(int i = 0; i<width; i++)
		{
			for(int j = limit-20; j<height; j++)
			{
		...
		//CRYSTALS
		float[][] perlinCrystals = noise.generateGradient(width, height, 10, 10);
				
		for(int i = 0; i<width; i++)
		{
			for(int j = 0; j<height; j++)
			{					
				if(perlinDirt[i][j]>0.25)
				{
					BlocType b = AllBlocTypes.instance.Dirt;
				...
				if(perlinDirt[i][j]>0.3 && perlinFossile[i][j]>0.4f)
				{
					Map.instance.mainLayer.blocs[i][j] = AllBlocTypes.instance.Fossile;
				}
						
				if(perlinCrystals[i][j] > 0.45 && perlinFossile[i][j]>0.45f && Map.instance.mainLayer.blocs[i][j] == AllBlocTypes.instance.Cobble)
				{
				...
				if(perlinCrystals[i][j] > 0.48 && perlinDirt[i][j]<-0.45f && Map.instance.mainLayer.blocs[i][j] == AllBlocTypes.instance.Cobble)
				{
		...
		float[][] perlinHoles3 = noise.generateGradient(width, height, 100, 60,0.5f,3);
		
		float[][] perlinHoles4 = noise.generateGradient(width, height, 300, 150);
		...
		for(int i = 0; i<width; i++)
		{
			for(int j = 0; j<height; j++)
			{
				...
				float p1 = perlinHoles3[i][j];
				float p2 = perlinHoles4[i][j]; 
				//float p3 = p1*Math.abs(p2+0.4f);
						//(float) Noise.noise((double)i*20/genwidth, (double)j*70/genwidth);
				
				//if(i<20)p1 = (p1*i+(perlinHoles3[width-1-i][j])*(20-i))/20;
				//if(i<20)p2 = (p2*i+(perlinHoles4[width-1-i][j])*(20-i))/20;
		...
		{
			Map.instance.mainLayer.blocs[0][i] = (byte) AllBlocTypes.instance.UnbreakId;
			Map.instance.mainLayer.blocs[1][i] = (byte) AllBlocTypes.instance.UnbreakId;
			
			Map.instance.mainLayer.blocs[width-2][i] = (byte) AllBlocTypes.instance.UnbreakId;
			Map.instance.mainLayer.blocs[width-1][i] = (byte) AllBlocTypes.instance.UnbreakId;
		}*/
		mainProgress = 1f;
	}
